<html>
    <head>
        <?php foreach($venue as $venue_item): ?>
        <title>GigTime | <?php echo $venue_item['user_name']; ?></title>
        <?php endforeach; ?>
        <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">

        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js"></script>
        <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js"></script>
        
        <link rel="stylesheet" type="text/css" href="<?php echo base_url('css/myProfileVenue.css'); ?>">
        <link rel="stylesheet" type="text/css" href="<?php echo base_url('css/dropdown.css'); ?>">

        <meta name="viewport" content="width=device-width, initial-scale=1">
    </head>
    <body>
        <div class="container">
            <?php include 'header.php'; ?> 
            
            <div id="map"></div>
            
            <div class="ozadje">
                <a href="<?php echo site_url('myProfileController/changeLocation_view'); ?>" class="btn btn-primary">Spremeni lokacijo</a>
                <div class="levo">
                    <form method="post" action="<?php echo site_url('myProfileController/changeVenueSettings'); ?>">
                        <?php foreach($venue as $venue_item): ?>
                        <h1><?php echo $venue_item['user_name']; ?></h1>

                        <h2>Naslov: <?php echo $venue_item['location']; ?></h2>

                        <h3>Kapaciteta: <input class="form-control" name="kapaciteta" placeholder="Dodaj kapaciteto prizorišča" value="<?php echo $venue_item['size']; ?>"></h3>

                        <h3>Na prostem/prostor: 

                        <div class="form-group">
                            <select class="form-control" name="indoor" id="exampleSelect1">
                                <?php if($venue_item['indoor'] == '0'){
                                        $beseda = "Na prostem";
                                        $cifra = 0;
                                    }else if($venue_item['indoor'] == '1'){
                                        $beseda = "V prostoru";
                                        $cifra = 1;
                                    }else{
                                        $beseda = "Oboje";
                                        $cifra = 2;
                                    }?>
                                <option selected="" value="<?php echo $cifra; ?>"><?php echo $beseda; ?></option>
                                <option value="0">Na prostem</option>
                                <option value="1">V prostoru</option>
                                <option value="2">Oboje</option>
                          </select>
                        </div>

                        </h3>

                        <div class="opis">
                            <label for="desc">Opis prizorišča</label>
                            <textarea class="form-control" id="desc" name="desc"><?php echo $venue_item['description']; ?></textarea>
                        </div>

                        <label for="contact">Kontaktne informacije</label>
                        <textarea id="contact" class="form-control" name="contact" placeholder="Vnesi kontaktne informacije"><?php echo $venue_item['contactVenue']; ?></textarea>
                        <?php endforeach; ?>

                        <button class="btn btn-primary" type="submit">Shrani spremembe</button>
                    </form>
                </div>
                
                <div class="desno">
                    <center>
                        <?php if($this->session->userdata('img')) { ?>
                            <img src="<?php echo base_url('uploads/'.$this->session->userdata('img')); ?>" class="profilka"/><br>
                        <?php 
                            }
                            else { 
                        ?>
                            <img src="<?php echo base_url('images/defaultProfile.jpg'); ?>" class="defaultprofilka" /><br>
                        <?php
                            }
                        ?>
                        
                        <form method="post" action="<?php echo site_url('myProfileController/newPicture'); ?>" enctype="multipart/form-data">
                            <div class="custom-file" id="customFileProfilka">
                                <input type="file" class="custom-file-input" id="customFile" name="userfile">
                                <label class="custom-file-label" for="customFile">Choose file</label>
                            </div>
                            <button class="btn btn-primary">Spremeni sliko</button>
                        </form>
                    </center>
                </div>
                
                <h2>Slike prizorišča</h2>
                <div class="slike">
                    
                    <div class="responsive">
                        <div class="gallery">
                            <center><h2>Dodaj novo sliko</h2></center>
                            <form method="post" action="<?php echo site_url('myProfileController/addNewPicture'); ?>" enctype="multipart/form-data">
                                <div class="custom-file">
                                    <input type="file" class="custom-file-input" id="customFile" name="userfile">
                                    <label class="custom-file-label" for="customFile">Choose file</label>
                                </div>

                                <input type="text" name="opis" class="form-control novaSlikca" placeholder="Dodaj opis slike"/>
                                <input type="date" class="form-control novaSlikca" name="datum"/><br>
                                <button type="submit" class="btn btn-primary slikeGumb">Dodaj sliko</button>
                            </form>
                        </div>
                    </div>
                    
                    <?php foreach($images as $image): ?>
                        <div class="responsive">
                            <div class="gallery">
                                <a target="_blank" href="<?php echo base_url('uploads/pictures/'.$image['picture_filename']); ?>">
                                    <center><img src="<?php echo base_url('uploads/pictures/'.$image['picture_filename']); ?>"></center>
                                </a>
                                <div class="desc"><?php echo $image['imgDesc']; ?></div>

                                <?php if($image['imgDate']!= '0000-00-00'){ ?>
                                <div class="imgDate"><?php echo $image['imgDate']; ?></div>
                                <?php }else{ ?>
                                <div></div><?php }; ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
                
            </div>
        </div>
        
        
        
        
        
        
        
        
        
        
        
        
        
        <script>
                function initMap() {
                    
                    var arr = <?php echo json_encode($venue); ?>;
                    
                    var myLatLng = {lat: parseFloat(arr[0].lat), lng: parseFloat(arr[0].lng)};

                    var map = new google.maps.Map(document.getElementById('map'), {
                        zoom: 15,
                        center: myLatLng
                    });


                    var marker = new google.maps.Marker({position: myLatLng, map: map});   
                        
                    google.maps.event.addListener(marker, 'click', getInfoCallback(map, contentString));
                        
                    
                    
                    
                    //odpre se infowindow
                    function getInfoCallback(map, content) {
                        var infowindow = new google.maps.InfoWindow({content: content});
                        return function() {
                            infowindow.close();
                            infowindow.setContent(content); 
                            infowindow.open(map, this);
                        };
                    }
                    
                }
            </script>
        
        <script>
            $(".custom-file-input").on("change", function() {
              var fileName = $(this).val().split("\\").pop();
              $(this).siblings(".custom-file-label").addClass("selected").html(fileName);
            });
        </script>
        <script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyC5kE3PjFpshQzI1eH_isjZq3tPGF-4Vq0&libraries=places&callback=initMap"></script>
    </body>
</html>